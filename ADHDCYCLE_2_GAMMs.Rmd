...
# - Make sure all outcomes are included
# - Update so that the time variable can be easily switched between cyclic_time and cyclic_time_imp_ov
# - Update the labels on the X axis of the cycle to switch between orders
...
gamm_output_folder <- "~/Library/CloudStorage/Box-Box/00 - CLEAR Lab (Locked Folders)/02 - Data Management, Analysis, and Papers/Studies_Projects/CYCLEADHD/03_analytic_projects/CYCADHD_PRIMARY/03_code_dataedits_output/"
...
# Run iterations with the sensitivity analysis dataset removing folks with messy hormones/cycles
```{r}
...
for (outcome_var in names(outcomes)) {
  ...
  # Log-transform the current outcome variable
  df_sens[[log_outcome_var]] <- log(df_sens[[outcome_var]] + 1)

  # Prepare data for the model using the specified time_var
  selected_vars <- c(time_var, log_outcome_var, "id")
  datSX <- df_sens[complete.cases(df_sens[selected_vars]), ]
  datSX$id <- as.factor(datSX$id)
  ...
  # Save the plot to a dynamically named .png file
  plot_filename <- paste0(gamm_output_folder, "gamplot_", log_outcome_var, "_", time_var, "_sensitivity_", current_date, ".png")
  ggsave(plot_filename, gamplot, width = 8, height = 6, dpi = 300)
