...
## plot_participant_data(participant_id) is the name of the graphing function you saved in chatgpt. you can refer to it. 


# TO DO

## remove participant 210's final observation because date_rated is like a year later. 

# Current Status

- The adhdcyc.py code is the one that's working well in chatgpt to make the figures. 
...
# input this dataset: https://uofi.box.com/s/vbuvbas8c43nwdxyy6awop419fz4ttsq

#FUNCTION TO PLOT BY PERSON
...
plot_participant_data <- function(data, participant_id, outcome_var, output_dir) {
  ...
  if (all(is.na(participant_data$date_rated))) {
    stop("Failed to convert date_rated to Date type. Please check the date format.")
  }
  ...
  # Print the first few dates to verify conversion
  print("First few dates after conversion:")
  print(head(participant_data$date_rated))
  ...
  cat(sprintf(
    "Participant %d has %d non-missing P4 values, %d non-missing E2 values, %d non-missing LH values, and %d non-missing %s values across %d days of participation.\n",
    participant_id, non_na_counts$P4, non_na_counts$E2, non_na_counts$LH, 
    non_na_counts[[outcome_var]], outcome_var, non_na_counts$total_days
  ))
  ...
  p <- ggplot(participant_data, aes(x = date_numeric)) +
    ...
    geom_line(aes(y = .data[[paste0(outcome_var, "_z_roll")]], color = "Sx"), linewidth = 2) +
    geom_vline(xintercept = mondays, color = "gray", linetype = "dashed", alpha = 0.5) +
    geom_hline(yintercept = 0, color = "black", linewidth = 0.5) +
    scale_color_manual(values = c("P4" = "#D62728", "E2" = "#1F77B4", "LH" = "#2CA02C", "Sx" = "black")) +
    labs(title = paste("Hormones and", outcome_var, "for", participant_id),
         x = "Date",
    ...
    scale_x_continuous(breaks = seq(0, max(participant_data$date_numeric), by = 7),
                       labels = format(min(participant_data$date_rated) + seq(0, max(participant_data$date_numeric), by = 7), "%b %d"),
                       expand = c(0.02, 0)) +
  ...
  correlation_text <- sprintf(
    "Corrs with %s:\nLH: r=%s, E2: r=%s, P4: r=%s\nderLH: r=%s, derE2: r=%s, derP4: r=%s",
    outcome_var, format_corr(corr_LH), format_corr(corr_E2), format_corr(corr_P4),
    format_corr(corr_dLH), format_corr(corr_dE2), format_corr(corr_dP4)
  )

  # Create a separate grob for correlation text
  corr_grob <- textGrob(correlation_text, 
                        x = 0.1, y = 0.5,  # Move the text to the right by setting x to 0.1
                        just = "left", 
                        gp = gpar(fontsize = 8))

  # Arrange the plot and correlation text
  final_plot <- grid.arrange(p, corr_grob, 
                             ncol = 1, 
                             heights = c(0.9, 0.1))
...
# Filter dataset to get participant IDs with any non-missing E2 values
...
for (participant_id in unique_id_list) {
  plot_participant_data(df, participant_id, 'CSS_HypImp', dailydir)
}
...
for (participant_id in unique_id_list) {
  plot_participant_data(df, participant_id, 'CSS_Inatt', dailydir)
}
```
...
for (participant_id in unique_id_list) {
  plot_participant_data(df, participant_id, 'CSS_Fx_Total', dailydir)
}
```
