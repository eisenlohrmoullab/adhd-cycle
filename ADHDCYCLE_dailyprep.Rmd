...
plot_DVs <- function(DV_list, dat) {
  ...
  dat <- dat %>%
    group_by(id) %>%
    mutate(E2.rescale = scale(E2, center = TRUE, scale = sd(E2, na.rm=TRUE)/0.66), #changed here
           P4.rescale = scale(P4, center = TRUE, scale = sd(P4, na.rm=TRUE)/0.66)) %>% #changed here
    ungroup()
