...
file_path <- "~/Library/CloudStorage/Box-Box/00 - CLEAR Lab (Locked Folders)/02 - Data Management, Analysis, and Papers/Studies_Projects/CYCLEADHD/03_analytic_projects/CYCADHD_PRIMARY/03_code_dataedits_output/2025-09-22/datasets_interim/adhd_daily_df_scaled_20250922.RData"
...
smm_output_folder <- "~/Library/CloudStorage/Box-Box/00 - CLEAR Lab (Locked Folders)/02 - Data Management, Analysis, and Papers/Studies_Projects/CYCLEADHD/03_analytic_projects/CYCADHD_PRIMARY/03_code_dataedits_output/2025-09-22/SMMs"
...
# Combine the base folder path and the current date to create the full path
dated_output_folder <- file.path(smm_output_folder, current_date)

# Check if the dated directory exists, and create it if it doesn't
if (!dir.exists(dated_output_folder)) {
  dir.create(dated_output_folder, recursive = TRUE)
  message(paste("Successfully created directory:", dated_output_folder))
} else {
  message(paste("Directory already exists:", dated_output_folder))
}
```
...
# You can easily switch the time variable for your models if needed
