...
final_df <- final_df %>%
  ...
  filter(!(id == 323 & date > as.Date("2023-01-14"))) %>%
...
#final_df %>% filter(id == 347) %>% select(id, date, E2, P4, LH, BDEFS_1, score_pinball)
```
...
for (row in 1:nrow(metrics_to_plot)) {
  ...
  for (person_id in ids_list) {
    ...
    if (should_facet) {
      ...
      # Horizontal line for P4 at y=82 if metric is raw, 3roll, or 5roll
