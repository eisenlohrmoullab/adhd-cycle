...
# Plot Correlations
```{r}
# --- Add this code chunk to your R Markdown file ---
...
generate_correlation_heatmap <- function(data, outcome_vars, id_var) {
  ...
  # Spearman correlation on person-mean scores
  ...
  for (i in 1:n_vars) {
    for (j in 1:n_vars) {
      if (i > j) { # Only calculate for the lower triangle
        ...
        # rmcorr requires a dataframe with no missing values for the pair
        rmcorr_data <- data %>%
          select(!!sym(id_var), !!sym(var1), !!sym(var2)) %>%
          drop_na()
        
        # Ensure there's enough data
        if (nrow(rmcorr_data) > 0) {
          rmcorr_result <- rmcorr(participant = !!sym(id_var), 
                                  measure1 = !!sym(var1), 
                                  measure2 = !!sym(var2), 
                                  dataset = rmcorr_data)
  ...
  plot <- ggcorrplot(
    ...
    colors = c("#6D9EC1", "white", "#E46726"), # Blue, white, orange color scheme
    ...
    theme(
      ...
      panel.grid.major = element_blank() # Clean up the background
    )
