...
final_df %>% filter(id == 323) %>% select(id, date, E2, P4, LH, BDEFS_1, score_pinball)
...
# Hormone Data Cleaning
# Calculate robust Z-scores for each hormone within each person

```{r}
df_with_robust_z <- df %>%
  group_by(id) %>%
  mutate(
    E2_robust_z = (0.6745 * (E2 - median(E2, na.rm = TRUE))) / mad(E2, na.rm = TRUE),
    P4_robust_z = (0.6745 * (P4 - median(P4, na.rm = TRUE))) / mad(P4, na.rm = TRUE),
    LH_robust_z = (0.6745 * (LH - median(LH, na.rm = TRUE))) / mad(LH, na.rm = TRUE)
  ) %>%
  ungroup()
```

# Visualize the robust Z-scores to identify outliers

```{r}

robust_z_plot <- df_with_robust_z %>%
  pivot_longer(cols = c(E2_robust_z, P4_robust_z, LH_robust_z), names_to = "hormone", values_to = "robust_z") %>%
  ggplot(aes(x = robust_z, fill = hormone)) +
  geom_histogram(bins = 50, alpha = 0.6, position = "identity") +
  facet_wrap(~ hormone, scales = "free") +
  labs(title = "Distribution of Robust Z-Scores for Hormones", x = "Robust Z-Score", y = "Count") +
  theme_light() + theme(legend.position = "none")
print (robust_z_plot)
ggsave(filename = file.path(output_folder, "robust_z_score_distributions.png"), plot = robust_z_plot, width = 10, height = 6, dpi = 300)

```

# Now you could easily filter out for potential outliers

```{r}
#outliers_mad <- df_with_robust_z %>% filter(abs(E2_robust_z) > 3.5 | abs(P4_robust_z) > 3.5)
```
...
e2_comparison_plot <- plot_distributions(df, "E2")
...
# 3. Do for P4
p4_comparison_plot <- plot_distributions(df, "P4")
...
# 3. Do for LH
LH_comparison_plot <- plot_distributions(df, "LH")
print(LH_comparison_plot)
ggsave(
  filename = file.path(output_folder, "LH_comparison_distributions.png"),
  plot = p4_comparison_plot, width = 10, height = 8
)
...
# 2. Define the corrected function
generate_correlation_heatmap <- function(data, outcome_vars, id_var) {
  
  n_vars <- length(outcome_vars)
  results_matrix <- matrix(NA, nrow = n_vars, ncol = n_vars,
                           dimnames = list(outcome_vars, outcome_vars))
  
  # --- Steps 2, 3, 4: Calculate all values ---
  
  # Between-Person (Upper)
  cat("Calculating between-person correlations...\n")
  person_means <- data %>%
    group_by(!!sym(id_var)) %>%
    summarise(across(all_of(outcome_vars), ~mean(., na.rm = TRUE)))
  between_corr <- cor(person_means[, outcome_vars], method = "spearman", use = "pairwise.complete.obs")
  results_matrix[upper.tri(results_matrix)] <- between_corr[upper.tri(between_corr)]
  
  # ICC (Diagonal)
  cat("Calculating Intra-Class Correlations...\n")
  for (i in 1:n_vars) {
    var <- outcome_vars[i]; formula <- as.formula(paste0(var, " ~ 1 + (1|", id_var, ")"))
    if (sum(!is.na(data[[var]])) > 1) {
      model_fit <- try(lmer(formula, data = data), silent = TRUE)
      if (!inherits(model_fit, "try-error")) {
        results_matrix[i, i] <- icc(model_fit)$ICC_adjusted
      }
    }
  }
  
  # Within-Person (Lower)
  cat("Calculating within-person correlations...\n")
  within_corr_matrix <- matrix(0, nrow = n_vars, ncol = n_vars,
                               dimnames = list(outcome_vars, outcome_vars))
    ...
    for (j in 1:n_vars) {
      if (i > j) {
        ...
        if (nrow(rmcorr_data) > 2) {
            rmcorr_result <- rmcorr(participant = id_var, measure1 = var1, measure2 = var2, dataset = rmcorr_data)
            results_matrix[i, j] <- rmcorr_result$r
            within_corr_matrix[i, j] <- within_corr_matrix[j, i] <- rmcorr_result$r
        }
      }
    }
  }
  
  # --- Step 5: Determine Clustering Order and Reorder the Matrix ---
  cat("Reordering matrix by clustering...\n")
  
  # --- THIS IS THE CORRECTED PART ---
  # Perform hierarchical clustering manually to get the correct order
  # 1. Calculate a distance matrix (1 - correlation)
  dist_matrix <- as.dist(1 - abs(within_corr_matrix))
  # 2. Perform clustering
  hclust_obj <- hclust(dist_matrix, method = "ward.D2")
  # 3. Get the reordered variable names
  corr_order <- rownames(within_corr_matrix)[hclust_obj$order]
  # --- END OF CORRECTION ---
  
  # Apply this order to your final composite matrix
  ...
  plot <- ggcorrplot(
    ...
    labs(
      caption = "Upper Triangle: Between-person Spearman ρ (on person-means)\nDiagonal: Intra-Class Correlation (ICC)\nLower Triangle: Within-person repeated measures r"
    ) +
    theme(
...
# Define your list of daily outcome variables
#my_dv_list <- c("CSS_Inatt", "CSS_HypImp", "BDEFS_Total", "UPPS_PU_avg", "DEBQ_Total")
#corrplotlist

# Generate the plot (assuming your data is in 'df' and ID is 'id')
correlation_heatmap <- generate_correlation_heatmap(data = df_scaled, 
...
ggsave(filename = file.path(output_folder, "multilevel_correlation_heatmap.png"), 
...
generate_correlation_heatmap <- function(data, outcome_vars, id_var) {
  ...
  # --- Step 1: Initialize the results matrix ---
  n_vars <- length(outcome_vars)
  results_matrix <- matrix(NA, nrow = n_vars, ncol = n_vars,
                           dimnames = list(outcome_vars, outcome_vars))
  
  # --- Step 2: Calculate Between-Person Correlations (Upper Triangle) ---
  cat("Calculating between-person correlations (upper triangle)...\n")
  person_means <- data %>%
    group_by(!!sym(id_var)) %>%
    summarise(across(all_of(outcome_vars), ~mean(., na.rm = TRUE)))
  ...
  # --- Step 3: Calculate Intra-Class Correlations (Diagonal) ---
  cat("Calculating Intra-Class Correlations (diagonal)...\n")
  for (i in 1:n_vars) {
    var <- outcome_vars[i]
    formula <- as.formula(paste0(var, " ~ 1 + (1|", id_var, ")"))
    
    # Ensure there's enough data to fit the model
    if (sum(!is.na(data[[var]])) > 1) {
      # Use try() to catch potential model convergence errors
      model_fit <- try(lmer(formula, data = data), silent = TRUE)
      if (!inherits(model_fit, "try-error")) {
        icc_val <- icc(model_fit)$ICC_adjusted
        results_matrix[i, i] <- icc_val
      }
    }
  }
  
  # --- Step 4: Calculate Within-Person Correlations (Lower Triangle) ---
  cat("Calculating within-person correlations (lower triangle)...\n")
  for (i in 1:n_vars) {
    for (j in 1:n_vars) {
      if (i > j) { # Only calculate for the lower triangle
        ...
        rmcorr_data <- data %>%
          select(all_of(c(id_var, var1, var2))) %>%
          drop_na()
        
        # Ensure there are enough paired observations for rmcorr
        if (nrow(rmcorr_data) > 2) {
            # --- THIS IS THE CORRECTED PART ---
            # rmcorr expects strings for participant and measure names, not symbols.
            rmcorr_result <- rmcorr(participant = id_var, 
                                    measure1 = var1, 
                                    measure2 = var2, 
                                    dataset = rmcorr_data)
            results_matrix[i, j] <- rmcorr_result$r
  ...
  # --- Step 5: Generate the Publication-Ready Plot ---
  cat("Generating plot...\n")
  
  plot <- ggcorrplot(
  corr = results_matrix,
  hclust = TRUE,          # Add this line to reorder by clustering
  hclust.method = "ward.D2", # A good clustering algorithm
    method = "square",
    type = "full",
    lab = TRUE, 
    lab_size = 3.5,
