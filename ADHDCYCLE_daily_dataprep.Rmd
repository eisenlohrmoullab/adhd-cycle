...
#rawdf <- read_csv("~/Library/CloudStorage/Box-Box/00 - CLEAR Lab (Locked Folders)/02 - Data Management, Analysis, and Papers/Studies_Projects/CYCLEADHD/02_datasets/CYCADHD_DAILY/01_raw_data/2024.04.24. Daily Master.csv") 
...
knapp_dates <- read_excel("~/Library/CloudStorage/Box-Box/00 - CLEAR Lab (Locked Folders)/02 - Data Management, Analysis, and Papers/Studies_Projects/CYCLEADHD/02_datasets/CYCADHD_DAILY/02_data_prep_workspace/2025-08-20/cycadhd_mensov_iso.xlsx")
...
# Merge in additional hormone data from Emily Knapp August 2025
#OLD NAMES (already changed): id	sample	Box #	Estradiol (pg/mL)	STERR	Progesterone (pg/mL)	STERR	LH (uIU/mL)	lh_sterr	hormone_assay_note	
# New names already changed: Changed names to id sample Box # Estradiol E2_stderr Progesterone P4_stderr LH lh_sterr hormone_assay_note

```{r}
finalhormonebatch <- read_excel("~/Library/CloudStorage/Box-Box/00 - CLEAR Lab (Locked Folders)/02 - Data Management, Analysis, and Papers/Studies_Projects/CYCLEADHD/02_datasets/CYCADHD_DAILY/02_data_prep_workspace/2025-08-20/Martel_IRB52576_Results_E2_P4_LH 7-23-2025.xlsx")

#View(finalhormonebatch)

#Drop the variable "Box #" and set id to dbl format
...
# Make a copy of the TubeNumber variable called "days_in_study"

finalhormonebatch <- finalhormonebatch %>%
  mutate(TubeNumber = as.numeric(TubeNumber)) %>% # Convert TubeNumber to numeric
  mutate(days_in_study = TubeNumber) # Create days_in_study variable
...
finalhormonebatch$TubeNumber <- as.numeric(finalhormonebatch$TubeNumber)
...
# Need to create a new variable daterated in the finalhormonebatch dataset based on the first morning daterated value for each id in the df dataset
...
first_day_in_study <- df %>%
  group_by(id) %>%
  summarize(first_day_in_study = min(date_rated)) %>%
  ungroup()
...
finalhormonebatch <- finalhormonebatch %>%
  mutate(date_rated = first_day_in_study + TubeNumber - 1)

# Check to see if it worked 
View(finalhormonebatch)

vis_miss(finalhormonebatch)
...
finalhormonebatch <- finalhormonebatch %>%
  group_by(id) %>%
  fill(date_rated) %>% # Fill in missing daterated values using TubeNumber as a counting index day variable within each id
  ungroup()
...
finalhormonebatch$date_rated <- as.Date(finalhormonebatch$date_rated, format = "%Y-%m-%d")
# Format daterated as ISO in both datasets
df$date_rated <- as.Date(df$date_rated, format = "%Y-%m-%d")

#View(df)

# Merge together the hormone data with the main dataset

# Merge finalhormonebatch with df by id and daterated only including observations that are in both datasets

df <- df %>%
  left_join(finalhormonebatch, by = c("id", "date_rated"))
