...
```{r}
updates <- read_xls("~/Library/CloudStorage/Box-Box/00 - CLEAR Lab (Locked Folders)/02 - Data Management, Analysis, and Papers/Studies_Projects/CYCLEADHD/02_datasets/CYCADHD_DAILY/02_data_prep_workspace/2025-08-26/adhdcyc_dateupdates_taem.xls")

View(updates)

#For each combination of id and date in the updates dataset, update the corresponding menses and ovtoday values in the df dataset; if the combination does not exist, make it exist by adding a new row with NA for all other variables except id, date, menses, and ovtoday
...
updates_min <- updates_min %>%
  group_by(id, date) %>%
  summarise(
    menses  = dplyr::last(na.omit(menses))  %||% NA_integer_,
    ovtoday = dplyr::last(na.omit(ovtoday)) %||% NA_integer_,
    .groups = "drop"
)

df <- df %>%
  mutate(date = as.Date(date)) %>%
  rows_upsert(updates_min, by = c("id","date")) %>%
  arrange(id, date)
...
dat <- df %>% select(id, date, menses, ovtoday, E2.zd, P4.zd, LH.zd)
dat3 <- df %>% select(id, date, menses, ovtoday, E2.zd.3roll, P4.zd.3roll, LH.zd.3roll)
dat5 <- df %>% select(id, date, menses, ovtoday, E2.zd.5roll, P4.zd.5roll, LH.zd.5roll)
...
dat.long.z3r = pivot_longer(dat3, cols = c("E2.zd.3roll", "P4.zd.3roll", "LH.zd.3roll"), names_to = "hormone", values_to = "value")
dat.long.z5r = pivot_longer(dat5, cols = c("E2.zd.5roll", "P4.zd.5roll", "LH.zd.5roll"), names_to = "hormone", values_to = "value")
...
#View dataset with only id 210
View(df %>% filter(id == 210))
```

```{r}
# how many menses rows exist vs have non-missing x-var used in the plot?
df %>% filter(StartPeriod %in% c(1L, TRUE)) %>% summarise(
  total_markers = n(),
  markers_with_date  = sum(!is.na(date)),       # if your ggplot x uses 'date'
  markers_with_drate = sum(!is.na(daterated))   # if ggplot x uses 'daterated'
)
...
for (i in ids_with_E2_list){
  ...
  out <- paste0("~/Library/CloudStorage/Box-Box/00 - CLEAR Lab (Locked Folders)/02 - Data Management, Analysis, and Papers/Studies_Projects/CYCLEADHD/03_analytic_projects/CYCADHD_PRIMARY/03_code_dataedits_output/daily_hormone_plots_zscored_20250825/zhormoneplot_3r_", i, ".png")
  ggsave(out, p, width = 10, height = 6, dpi = 300)
}
...
for (i in ids_with_E2_list){
  ...
  out <- paste0("~/Library/CloudStorage/Box-Box/00 - CLEAR Lab (Locked Folders)/02 - Data Management, Analysis, and Papers/Studies_Projects/CYCLEADHD/03_analytic_projects/CYCADHD_PRIMARY/03_code_dataedits_output/daily_hormone_plots_zscored_20250825/zhormoneplot_5r_", i, ".png")
  ggsave(out, p, width = 10, height = 6, dpi = 300)
}
...
df_for_pacts <- df %>%
  select(id, date, ovtoday, menses, E2, P4, LH, DRSP_1, DRSP_2, DRSP_3, DRSP_4, DRSP_5, DRSP_6, DRSP_7, DRSP_8, DEBQ_avg, CSS_Inatt_Count, CSS_Imp_Count, CSS_Hyp_Count, BDEFS_WM_avg, BDEFS_RI_avg, UPPS_NU_avg, UPPS_Persev_avg, UPPS_Premed_avg, UPPS_Sens_avg, UPPS_PU_avg, score_pinball, score_robot)
...
#cycledata_check(df_scaled, c("DRSP_1", "BDEFS_WM_avg"))
#summary_ovulation(df_scaled)

cycle_plot(
  df_scaled,
  symptom="CSS_Inatt_Count",
  centering = "ovulation",
  include_impute = TRUE,
...
cycle_df_scaled$CSS_Inatt_Count_Log = log(cycle_df_scaled$CSS_Inatt_Count + 1) #log-transforming our outcome variable symptom

# Run GAMM for Inattention - cyclic time

selected_vars <- c("cyclic_time", "CSS_Inatt_Count_Log" )
datSX <- cycle_df_scaled[complete.cases(cycle_df_scaled[selected_vars]), ]

count_rows_ids(datSX)

datSX$id = as.factor(datSX$id) # ALWAYS factor id before putting it in a gam formula

gamm <- mgcv::gam(
  CSS_Inatt_Count_Log ~ 
    s(cyclic_time, bs = 'cc', k = 10) + 
    s(id, bs = 're') + 
    s(id, cyclic_time, bs = c('re','cc'), k = c(NA, 10)),
  data = datSX, 
...
plotdat <- expand.grid(cyclic_time = seq(-1, 1, by = 0.05),
...
pred <- marginaleffects::predictions(gamm, newdata = plotdat, type = "response", transform = function(x) exp(x) - 1) # applying a transform function, to undo to the log transformation on symptom. The transform can be removed if your outcome was not log-transformed 
plotdat$estimate = pred$estimate
plotdat$conf.low = pred$conf.low
plotdat$conf.high = pred$conf.high

# Plotting
gamplot <- ggplot(plotdat, aes(x = cyclic_time, y = estimate)) +
  scale_x_continuous(limits = c(-1, 1), breaks = seq(-1, 1, by = 0.50), 
                     labels = c("0%L", "50%L", "Menses Onset", "50%F", "Ovulation")) +
  labs(x = "", y = "CSS Inattention Sx Count") + # You can change the y-axis label to reflect your outcome
...
write.csv(df, "~/Library/CloudStorage/Box-Box/00 - CLEAR Lab (Locked Folders)/02 - Data Management, Analysis, and Papers/Studies_Projects/CYCLEADHD/03_analytic_projects/CYCADHD_PRIMARY/03_code_dataedits_output/adhd_daily_df_20250826.csv", row.names = FALSE)
