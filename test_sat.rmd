...
# This chunk sets global options and loads the custom function.
source("functions.R") 
...
raw_daily_prep <- raw_daily %>%
  ...
  # reimport supp_hormones but only import id, date, E2, P4, LH
supp_hormones_prep <- read_csv(path_supp_hormones, show_col_types = FALSE) %>%
  select(id, date, E2, P4, LH) %>%
  rename(daterated = date, id = id) %>%
