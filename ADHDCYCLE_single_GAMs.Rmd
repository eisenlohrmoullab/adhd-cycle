---
title: "ADHDCYCLE GAM Analysis (Revamped & Fast)"
...
# --- Load Dataset ---
# Assumes 'cycle_df_scaled' is loaded.
...
# This creates a "named vector" in R called 'outcomes'. This is a highly
# efficient way to create a lookup list.
#
# The structure is:
# "raw_column_name_in_your_data" = "Pretty Label for Plots"
#
# - The 'name' (left side) MUST EXACTLY match the column name in your data frame.
# - The 'value' (right side) is the user-friendly label you want to see in your reports.

outcomes <- c(
    # These are the outcomes currently active in the analysis.
    ...
    # --- Inactive Outcomes ---
    # The lines below are "commented out" with a '#' symbol. This means R will ignore
    # them. To include any of these variables in your analysis, simply remove the '#'
...
for (current_id in unique_ids) {
  ...
  # --- 1. MODELING STAGE: Run all models for the current participant ---
  ...
  for (current_time_var in time_vars) {
    for (outcome_var in names(outcomes)) {
      ...
      tryCatch({
        ...
        # Add all necessary identifiers to the prediction dataframe
        pred$outcome_label <- outcomes[outcome_var]
        pred$time_var_name <- current_time_var
        pred$p.value <- p_value
        pred$raw_range <- max(pred$estimate) - min(pred$estimate) # 💡 Calculate raw range here
  ...
  if (length(all_predictions) == 0) { message(paste("--- No successful models for ID:", current_id, ". Skipping PDF. ---")); next }
  
  pdf_filename <- file.path(report_folder, paste0("Full_Report_ID_", current_id, ".pdf"))
  pdf(pdf_filename, width = 8.5, height = 11)

  # --- Page 1+: Create a paginated, transposed summary table ---
  summary_df_wide <- dplyr::bind_rows(significance_results) %>%
    left_join(outcome_categories, by = "variable") %>%
    mutate(stars = case_when(p.value < 0.001 ~ "***", p.value < 0.01  ~ "**", p.value < 0.05  ~ "*", TRUE ~ "")) %>%
    select(Category = category, Outcome = outcome_label, `Time Variable` = time_var, P.Value = stars) %>%
    pivot_wider(names_from = Outcome, values_from = P.Value, values_fill = "N/A") %>%
    arrange(Category, `Time Variable`)

  outcome_colnames <- names(summary_df_wide)[3:ncol(summary_df_wide)]
  col_chunks <- split(outcome_colnames, ceiling(seq_along(outcome_colnames) / table_cols_per_page))
  
  for (i in seq_along(col_chunks)) {
    chunk_cols <- c("Category", "Time Variable", col_chunks[[i]])
    table_chunk <- summary_df_wide[, chunk_cols]
    table_theme <- ttheme_default(base_size = 7, core = list(fg_params = list(hjust = 0, x = 0.05)), colhead = list(fg_params = list(hjust = 0, x = 0.05)))
    table_grob <- tableGrob(table_chunk, rows = NULL, theme = table_theme)
    title_grob <- textGrob(paste("Significance of Cycle Effects for ID:", current_id, "(Page", i, "of", length(col_chunks), ")"), gp = gpar(fontsize = 14, fontface = "bold"))
    footnote_grob <- textGrob("* p < 0.05, ** p < 0.01, *** p < 0.001", gp = gpar(fontsize = 9, fontface = "italic"), hjust = 1)
    grid.arrange(title_grob, table_grob, footnote_grob, ncol = 1, heights = unit(c(0.05, 0.9, 0.05), "npc"))
  }
  
  # --- Subsequent Pages: Create the plots with consistent sizing ---
  combined_plot_data <- dplyr::bind_rows(all_predictions)

  # Helper function to create one fully annotated plot
  create_annotated_plot <- function(plot_df, time_var, menses_locs, ov_loc, axis_lbls) {
    ...
    # 💡 Simplified annotation without the Z-score
    ...
    ggplot(plot_df, aes(x = .data[[time_var]], y = estimate)) +
  ...
  for (current_time_var in time_vars) {
    ...
    for (page_name in names(page_definitions)) {
      ...
      plot_list <- lapply(page_outcomes, function(outcome) {
        ...
        create_annotated_plot(plot_df_single, current_time_var, menses_intercepts, ovulation_intercept, axis_labels)
      })
  ...
  dev.off()
  message(paste("--- ✅ Saved Full Report for ID:", current_id, "---"))
}
```
