...
# This chunk sets global options and loads custom functions.
# --- knitr Options ---
...
# FIX: Use uppercase hormone names to match the final dataframe
hormlist <- c("E2", "P4", "LH")
...
supp_hormones_prep <- supp_hormones %>%
  rename(daterated = date) %>% # E2, P4, LH names are already correct in this file
  mutate(id = as.numeric(id), daterated = ymd(daterated))

final_hormone_batch_prep <- final_hormone_batch %>%
  janitor::clean_names() %>%
  # FIX: Rename to the standard uppercase format
  rename(
    E2 = estradiol,
    P4 = progesterone,
    LH = lh,
...
# --- 4. Build Final 'df' with a Sequential Join and Coalesce ---
df <- raw_daily_combined %>%
  # Select only the columns we need before the joins
  select(id, daterated, E2, P4, LH, starts_with(c("css_", "bdefs_", "drsp_", "upps_", "debq_", "score_"))) %>%
  
  # Step A: Join supplemental hormones and coalesce
  ...
  select(-ends_with(".main"), -ends_with(".supp")) %>%

  # Step B: Join final hormone batch and coalesce again
  left_join(final_hormone_batch_prep, by = c("id", "daterated"), suffix = c("", ".final")) %>%
  mutate(
    E2 = coalesce(E2.final, E2),
    P4 = coalesce(P4.final, P4),
    LH = coalesce(LH.final, LH)
  ) %>%
...
# but is not part of the main analysis pipeline. Set eval=FALSE to prevent it from running.
...
# This helper function checks a source dataframe for hormone data for our list of missing IDs.
check_source_for_hormones <- function(source_df, source_name, ids_to_check) {
  ids_to_check <- as.numeric(ids_to_check)
  # Use cleaned names for checking
  hormone_cols <- c("e2", "p4", "lh")
  
  # Clean the names of the source df for the check
  ...
  summary_check <- source_df_clean %>%
    ...
    group_by(id) %>%
    summarise(
      e2_count = sum(!is.na(e2)), p4_count = sum(!is.na(p4)), lh_count = sum(!is.na(lh))
    ) %>%
