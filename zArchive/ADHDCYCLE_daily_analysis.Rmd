...
gamm_E2 <- gam(E2log ~ s(id, bs = "re") + 
                 s(id, scaled_cycleday_ov, bs = "re") + 
                 s(scaled_cycleday_ov), 
               data = df, method = "REML")
...
E2dat <- expand.grid(scaled_cycleday_ov = seq(-1, 1, by = 0.1), id = 0)
...
# Create the E2 plot
E2plot <- ggplot(E2dat, aes(x = scaled_cycleday_ov, y = estimate)) +
  scale_x_continuous(
    limits = c(-1, 1),
...
print(E2plot)
ggsave(
  filename = file.path(output_folder, "E2_gamm_plot.pdf"),
  plot = E2plot,
  width = 10,
  height = 8,
  dpi = 300,
...
gamm_P4 <- gam(P4log ~ s(id, bs = "re") + 
                 s(scaled_cycleday_ov, id, bs = "re") + 
                 s(scaled_cycleday_ov, k = 20),
               data = df, method = "REML")
...
P4dat <- expand.grid(scaled_cycleday_ov = seq(-1, 1, by = 0.1), id = 0)
...
P4plot <- ggplot(P4dat, aes(x = scaled_cycleday_ov, y = estimate)) +
  scale_x_continuous(
    limits = c(-1, 1),
    breaks = seq(-1, 1, by = 0.50),
    labels = c("Menses", "50%F", "Ovulation", "50%L", "Menses")
...
print(P4plot)
ggsave(
  filename = file.path(output_folder, "P4_gamm_plot.pdf"),
  plot = P4plot,
  width = 10,
  height = 8,
  dpi = 300,
...
LHdat <- expand.grid(scaled_cycleday_ov = seq(-1, 1, by = 0.1), id = 0)
...
LHplot <- ggplot(LHdat, aes(x = scaled_cycleday_ov, y = estimate)) +
  scale_x_continuous(
    limits = c(-1, 1),
    breaks = seq(-1, 1, by = 0.50),
    labels = c("Menses", "50%F", "Ovulation", "50%L", "Menses")
  ) +
  labs(x = "", y = "Salivary Luteinizing Hormone") +
  geom_rect(aes(xmin = -0.04, xmax = 0.04, ymin = -Inf, ymax = Inf),
            fill = "grey70", alpha = 0.2, color = "white") +
  geom_rect(aes(xmin = 0.92, xmax = 1, ymin = -Inf, ymax = Inf),
            fill = "grey87", alpha = 0.2, color = "white") +
...
print(LHplot)
ggsave(
  filename = file.path(output_folder, "LH_gamm_plot.pdf"),
  plot = LHplot,
  width = 10,
  height = 8,
  dpi = 300,
...
# merge e2dat, p4dat, lhdat
E2dat$variable <- "E2"
P4dat$variable <- "P4"
LHdat$variable <- "LH"
all_dat_orig <- rbind(E2dat, P4dat, LHdat)


#Redo graph scaling each hormone so they fit togethr well on the graph
E2dat$estimate <- scale(E2dat$estimate)
P4dat$estimate <- scale(P4dat$estimate)
LHdat$estimate <- scale(LHdat$estimate)
all_dat <- rbind(E2dat, P4dat, LHdat)

# Create a combined plot
combined_plot <- ggplot(all_dat, aes(x = scaled_cycleday_ov, y = estimate, color = variable)) +
  scale_x_continuous(
    limits = c(-1, 1),
    breaks = seq(-1, 1, by = 0.50),
    labels = c("Menses", "50%F", "Ovulation", "50%L", "Menses")
  ) +
  labs(x = "", y = "Salivary Hormone Levels") +
  geom_rect(aes(xmin = -0.04, xmax = 0.04, ymin = -Inf, ymax = Inf),
            fill = "grey70", alpha = 0.2, color = "white") +
  geom_rect(aes(xmin = 0.92, xmax = 1, ymin = -Inf, ymax = Inf),
            fill = "grey87", alpha = 0.2, color = "white") +
...
ggsave(
  filename = file.path(output_folder, "combined_gamm_plot.pdf"),
  plot = combined_plot,
  width = 10,
  height = 8,
  dpi = 300,
...
gamm_models_plots <- function(dv, df, output_folder, plot_path) {
  ...
  gamm_model <- gam(as.formula(paste0(log_dv, " ~ s(id, bs = 're') + s(scaled_cycleday_ov, id, bs = 're') + s(scaled_cycleday_ov, k = 30)")), 
                     data = df, method = "REML")
  ...
  scaled_cycleday_ov_row <- which(rownames(model_summary$s.table) == "s(scaled_cycleday_ov)")
  p_value <- model_summary$s.table[scaled_cycleday_ov_row, "p-value"]
  edf <- model_summary$s.table[scaled_cycleday_ov_row, "edf"]
  ...
  pred_data <- expand.grid(scaled_cycleday_ov = seq(-1, 1, by = 0.1), id = 0)
  ...
  dv_plot <- ggplot(pred_data, aes(x = scaled_cycleday_ov)) +
    ...
    geom_line(data = E2dat, aes(x = scaled_cycleday_ov, y = scale(estimate), color = "E2"), size = 1, linetype = "dashed") +
    geom_line(data = P4dat, aes(x = scaled_cycleday_ov, y = scale(estimate), color = "P4"), size = 1, linetype = "dashed") +
  ...
  dv_plot_orig <- ggplot(pred_data, aes(x = scaled_cycleday_ov)) +
    scale_x_continuous(limits = c(-1, 1), breaks = seq(-1, 1, by = 0.50), 
