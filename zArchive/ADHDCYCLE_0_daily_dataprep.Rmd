...
# Calculate sample-standardized values for E2, P4, and LH
```{r}

# Create new variables for sample-standardized E2, P4, and LH
df <- df %>%
  mutate(
    E2szd = (E2 - mean(E2, na.rm = TRUE))
           / sd(E2, na.rm = TRUE),
    P4szd = (P4 - mean(P4, na.rm = TRUE))
           / sd(P4, na.rm = TRUE),
    LHszd = (LH - mean(LH, na.rm = TRUE))
           / sd(LH, na.rm = TRUE)
  )

# See histograms for all 
hist(df$E2szd, breaks = 20, main = "Histogram of E2szd", xlab = "E2szd", col = "lightblue")
hist(df$P4szd, breaks = 20, main = "Histogram of P4szd", xlab = "P4szd", col = "lightblue")
hist(df$LHszd, breaks = 20, main = "Histogram of LHszd", xlab = "LHszd", col = "lightblue")


# Save those variables

dat4 <- df %>% select(id, date, menses, ovtoday, E2szd, P4szd, LHszd)
...
dat.long.szd = pivot_longer(dat4, cols = c("E2szd", "P4szd", "LHszd"), names_to = "hormone", values_to = "value")
...
```{r}
# Create a list of all the unique ids in the dataset
ids_list <- unique(df$id)
```


# SAMPLE-STANDARDIZED z-scored plots - +3roll
...
for (i in ids_list){
  # Get all relevant data for this participant
  base_df <- dat.long.szd %>%
    filter(id == i) %>%
    select(id, date, menses, ovtoday) %>%
    distinct() %>%
    filter(!is.na(date)) %>%
  ...
  # Ensure a complete sequence of dates for this participant and hormone
  all_dates <- seq(min(base_df$date), max(base_df$date), by = "day")
  complete_df <- expand.grid(date = all_dates, hormone = unique(dat.long.szd$hormone)) %>%
    mutate(id = i) %>%
    left_join(dat.long.szd %>% filter(id == i), by = c("id", "date", "hormone"))

  # Do NOT interpolate missing values! The rolling mean will be NA if any day in the window is NA.
  complete_df <- complete_df %>%
    arrange(hormone, date) %>%
    group_by(hormone) %>%
    mutate(rollmean3 = zoo::rollapply(value, width = 3, FUN = mean, align = "center", fill = NA, partial = TRUE, na.rm = FALSE)) %>%
    ungroup()

  # Prepare vertical lines for menses/ovulation
  menses_df  <- base_df %>% filter(menses == 1)  %>% distinct(date) %>% transmute(xintercept = date)
  ov_df      <- base_df %>% filter(ovtoday == 1) %>% distinct(date) %>% transmute(xintercept = date)

  # Plot: dots and lines for 3-day rolling mean only
  p <- ggplot() +
    ...
    ggtitle(i, subtitle = "dots/lines = 3-day rolling mean of zscore; missing values NOT filled; red = menses, purple = post-LH")

  # Only add points/lines if rolling mean data is present
  if(any(!is.na(complete_df$rollmean3))){
    p <- p +
      geom_point(data = complete_df, aes(x = date, y = rollmean3, color = hormone, group = hormone), size = 2, alpha = 0.85) +
      geom_line(data = complete_df, aes(x = date, y = rollmean3, color = hormone, group = hormone), linewidth = 1.2)
  }

  print(p)
  out <- paste0("~/Library/CloudStorage/Box-Box/00 - CLEAR Lab (Locked Folders)/02 - Data Management, Analysis, and Papers/Studies_Projects/CYCLEADHD/03_analytic_projects/CYCADHD_PRIMARY/03_code_dataedits_output/daily_hormone_plots_SAMPLEzscored_20250926/samplezhormoneplot_", i, ".png")
