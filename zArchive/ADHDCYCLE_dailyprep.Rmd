...
df <- df[, c("id", "date_rated", "E2", "P4", "LH", "StartPeriod", "PosLHTest", 
...
write.csv(df, "/Users/taem-macbook-air/Library/CloudStorage/Box-Box/00 - CLEAR Lab (Locked Folders)/02 - Data Management, Analysis, and Papers/Studies_Projects/CYCLEADHD/03_analytic_projects/CYCADHD_PRIMARY/03_code_dataedits_output/2024-10-03/adhd_daily_df_20241003_L2N85.csv", row.names = FALSE)
...
```{r}
# Load your CSV file (adjust the path to your actual file location)
df <- read_csv("~/Library/CloudStorage/Box-Box/00 - CLEAR Lab (Locked Folders)/02 - Data Management, Analysis, and Papers/Studies_Projects/CYCLEADHD/03_analytic_projects/CYCADHD_PRIMARY/03_code_dataedits_output/2024-10-03/adhd_daily_df_20241003_L2N85.csv")
```
...
# Looping through hormone effects 
...
#GAMM .zd and Derivative effects of Hormones
...
#GAMM .zd and Derivative effects of Hormones - CSS_HypImp (ZERO INF POISS)

```{r}

hist(df$CSS_HypImp)
df$CSS_HypImplog <- log(df$CSS_HypImp+1)
hist(df$CSS_HypImplog)

gamm_hormzd_CSS_HypImp <- gam(
  CSS_HypImp_Count ~ s(id, bs = "re") + s(E2zd, k=10) + s(E2zd, id, bs = "re") +
    s(P4zd, k=10) + s(P4zd, id, bs = "re") + ti(E2zd, P4zd, k=10),
  data = df,
  family = ziP(),
  method = "REML"
)
...
# Summary and plot of the model
summary(gamm_hormzd_CSS_HypImp)


# Save the model summary as a text file
summary_output <- capture.output(summary(gamm_hormzd_CSS_HypImp))
summary_file_path <- "/Users/taem-macbook-air/Library/CloudStorage/Box-Box/00 - CLEAR Lab (Locked Folders)/02 - Data Management, Analysis, and Papers/Studies_Projects/CYCLEADHD/03_analytic_projects/CYCADHD_PRIMARY/03_code_dataedits_output/2024-09-30/EP_Models_Plots/CSS_HypImp_model_EP_summary.txt"
writeLines(summary_output, summary_file_path)
cat("Model summary saved to:", summary_file_path, "\n")

plot.gam(gamm_hormzd_CSS_HypImp, select=2) #Estrogen
plot.gam(gamm_hormzd_CSS_HypImp, select=4) #Progesterone
plot.gam(gamm_hormzd_CSS_HypImp, select=7) #Estrogen CHANGE
plot.gam(gamm_hormzd_CSS_HypImp, select=8) #Progesterone CHANGE


# Simpler model without random effects for visualization
gamm_horm_CSS_HypImp_simple <- gam(
  CSS_HypImp ~ s(E2zd) + s(P4zd) + ti(E2zd, P4zd),
  data = df,
  family = gaussian,
  method = "REML"
)
...
# Predict values
grid$predicted <- predict(gamm_horm_CSS_HypImp_simple,
                          newdata = grid,
                          type = "response", transform = function(x) exp(x) - 1))
...
ggplot() +
  ...
  scale_fill_gradient2(
    ...
    mid = "white",
    high = "red",
    midpoint = mean(grid$predicted)) +
  labs(x = "E2zd", y = "P4zd", fill = "Outcome") +
  theme_minimal() +
  ggtitle("Predicting CSS_HypImp from E2 and P4 (Person Standardized)")

ggsave(filename = "/Users/taem-macbook-air/Library/CloudStorage/Box-Box/00 - CLEAR Lab (Locked Folders)/02 - Data Management, Analysis, and Papers/Studies_Projects/CYCLEADHD/03_analytic_projects/CYCADHD_PRIMARY/03_code_dataedits_output/2024-09-30/EP_Models_Plots/CSS_Inatt.png", 
       plot = last_plot(), 
       width = 8, 
       height = 6)
...
#GAMM .zd and Derivative effects of Hormones - TEMPLATE 
...
gamm_hormzd_CSS_HypImp <- gam(CSS_HypImplog ~ s(id, bs = "re") + s(E2zd) + s(E2zd, id, bs = "re") + s(P4zd) + s(P4zd, id, bs = "re") + ti(E2zd, P4zd), data = df, method = "REML")

summary(gamm_hormzd_CSS_HypImp)

plot.gam(gamm_hormzd_CSS_HypImp, select=2) #Plot E2zd
plot.gam(gamm_hormzd_CSS_HypImp, select=7) #Plot E2zch
plot.gam(gamm_hormzd_CSS_HypImp, select=4) #Plot P4zd
plot.gam(gamm_hormzd_CSS_HypImp, select=8) #Plot E2zch

#tab_model(gamm_hormzd_CSS_HypImp)

# CONTOUR PLOT for interaction of E2zd and P4zd

# Refit the model without random effects for visualization
gamm_horm_CSS_HypImp_simple <- gam(CSS_HypImplog ~ s(E2zd) + s(P4zd) + ti(E2zd, P4zd), data = df, method = "REML")

# Create a data frame for plotting
grid$predicted <- predict(gamm_horm_CSS_HypImp_simple, newdata = grid, transform = function(x) exp(x) - 1)

# Create contour plot with color gradients
ggplot(grid, aes(x = E2zd, y = P4zd, z = predicted)) +
  geom_tile(aes(fill = predicted)) +
  geom_contour(color = "black") +
  scale_fill_gradient2(low = "blue", mid = "white", high = "red", midpoint=mean(df$CSS_HypImplog)) +
  labs(x = "E2zd", y = "P4zd", fill = "Outcome") +
  theme_minimal()+
  ggtitle("Predicting CSS_HypImp from E2 and P4 (Person Standardized)")
