run_smm_cyclic <- function(
  ...
  for (i in seq_along(id_vec)) {
    ...
    if (nrow(person_data) >= 10) {
      fit <- tryCatch(mgcv::gam(as.formula(glue("{full_outcome} ~ s({time_var}, k = {k_smooth})")), data = person_data), error = function(e) NULL)
      if (!is.null(fit)) trajectory_matrix[i, ] <- predict(fit, newdata = grid)
    }
  }
  ...
  for (i in 1:d_inter) {
    ...
    for (k in 1:g) {
      ...
      models[[k]] <- gamm4::gamm4(as.formula(glue("{full_outcome} ~ s({time_var}, bs = 'cc', k = {k_smooth})")), random = as.formula(glue("~ (1 + {time_var} | id)")), data = dat_k, na.action = na.omit, knots = knots_list)
    ...
    df <- df %>% dplyr::select(-any_of("group")) %>% dplyr::left_join(group_map, by = "id")
  ...
  grp_counts <- df %>% dplyr::group_by(group) %>% dplyr::summarise(n = dplyr::n(), N = dplyr::n_distinct(id), .groups = "drop")
  df <- df %>% dplyr::left_join(grp_counts, by = "group") %>% dplyr::mutate(group_lab = factor(paste0(group, " (N=", N, ", n=", n, ")"), levels = paste0(sort(unique(group)), " (N=", grp_counts$N[match(sort(unique(group)), grp_counts$group)], ", n=", grp_counts$n[match(sort(unique(group)), grp_counts$group)], ")")))
  ...
  if (plot) {
    ...
    p_roll <- ggplot(summary_roll, aes(x = cycleday_5perc, y = mean_roll, color = group_lab, fill = group_lab)) +
      ...
      labs(title = paste(plot_label, "Rolling Avg (G =", g, ")"), x = "Percentage of Phase Elapsed", y = "Rolling Mean Centered", color = "Group") +
      theme_minimal()
    ...
    p_centered <- ggplot(summary_centered, aes(x = cycleday_5perc, y = mean_centered, color = group_lab, fill = group_lab)) +
      ...
      scale_x_continuous(breaks = x_breaks, labels = x_labels, limits = c(0, 1)) +
      labs(title = paste(plot_label, ": Person-Centered (G =", g, ")"), x = "Percentage of Phase Elapsed", y = "Mean Centered", color = "Group") +
      theme_minimal()
    ...
    p_mean <- ggplot(summary_mean, aes(x = cycleday_5perc, y = mean_outcome, color = group_lab, fill = group_lab)) +
      ...
      scale_x_continuous(breaks = x_breaks, labels = x_labels, limits = c(0, 1)) +
      labs(title = paste(plot_label, ": Mean (G =", g, ")"), x = "Percentage of Phase Elapsed", y = "Mean Outcome", color = "Group") +
      theme_minimal()
  }

  return(list(class = final_class, LLK = LLK, BIC = BIC, summary = df %>% dplyr::distinct(id, .keep_all = TRUE) %>% dplyr::group_by(group) %>% dplyr::summarise(n = dplyr::n(), mean_outcome = mean(.data[[outcome]], na.rm = TRUE), sd_outcome = sd(.data[[outcome]], na.rm = TRUE), .groups = "drop"), plots = list(plot_roll = p_roll, plot_centered = p_centered, plot_mean = p_mean)))
}